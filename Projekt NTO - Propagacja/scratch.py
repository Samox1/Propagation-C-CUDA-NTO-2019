...
for i in range(x):
    for o in range(y):
        h_z_kappa[i, o] = np.exp(1j*k*(pow((i-(x/2))*sampling, 2) + pow((o-(y/2))*sampling, 2))/(2*z))  #DOBRA LINIJKA

h_z = h_z_kappa * (np.exp(1j * k * z) / (1j * lam * z))    #DOBRA LINIJKA
...
scipy.misc.imsave("u_in_fft.bmp", u_in_fft[int(mx/2):x-int(mx/2), int(my/2):y-int(my/2)])
